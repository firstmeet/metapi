...
本部署分支基于上游项目 [Metapi](https://github.com/cita-777/metapi) 源码进行定制开发。当前仓库在上游能力基础上新增了 API Key 套餐余额管理：

- 可为 API Key 账号启用定时余额探测，默认请求上游的 `/v1/usage` 接口，也可配置自定义路径
- 将探测到的套餐余额、单位、有效状态和最近错误写入账号状态，并复用系统余额刷新计划
- 当套餐失效或余额低于安全阈值时，自动将该 API Key 连接排除在智能路由之外；余额恢复后自动重新参与路由
- 账号管理页支持配置探测路径与安全阈值，并展示最近探测结果

详细接入方式见 [上游接入](./docs/upstream-integration.md#3-api-key代理--可选套餐余额探测)。
