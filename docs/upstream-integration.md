...
##### 3. API Key（代理 / 可选套餐余额探测）

- **适用场景：** 使用 Base URL + API Key 调用模型，或聚合多个带独立套餐余额的 API Key
- **填写方式：**
  - 在「API Token」字段填入：`sk-xxxxxxxxxxxxxx`
- **可选余额探测：** 如果上游提供 Bearer Key 可访问的余额接口，可启用「定时查询套餐余额」，默认路径为 `/v1/usage`
- **查询周期：** 复用设置页的「余额刷新 Cron」（默认每小时）；长任务场景可设为 `*/5 * * * *`，并为单次任务预留足够的安全余额
- **路由保护：** 已成功探测且余额小于或等于安全阈值时，该连接会暂停参与路由；余额恢复后自动重新加入
- **限制：** API Key 仍无法使用签到、账号令牌管理等 Session 功能

余额接口支持以下字段（按顺序取第一个有效值）：

- 余额：`remaining` / `quota.remaining` / `balance`
- 单位：`unit` / `quota.unit`，默认 `USD`
- 有效性：`is_active` / `isValid`，默认 `true`
