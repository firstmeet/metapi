...
function createTokenForm(credentialMode: "session" | "apikey" = "session") {
  return {
    ...
    usageProbeEnabled: false,
    usageProbePath: "/v1/usage",
    usageMinRemaining: "1",
  };
}
...
export default function Accounts() {
    ...
    try {
      const result = await api.addAccount({
        ...
        apiKeyUsage:
          credentialMode === "apikey"
            ? {
                enabled: tokenForm.usageProbeEnabled,
                path: tokenForm.usageProbePath.trim() || "/v1/usage",
                minRemaining: Math.max(
                  0,
                  Number(tokenForm.usageMinRemaining) || 0,
                ),
              }
            : undefined,
      });
  ...
  const extractApiKeyUsageConfig = (account: any) => {
    const usage = parseAccountExtraConfig(account)?.apiKeyUsage || {};
    return {
      enabled: usage.enabled === true,
      path:
        typeof usage.path === "string" && usage.path.trim()
          ? usage.path.trim()
          : "/v1/usage",
      minRemaining:
        typeof usage.minRemaining === "number" &&
        Number.isFinite(usage.minRemaining)
          ? String(usage.minRemaining)
          : "1",
    };
  };
  ...
  const openEditPanel = (account: any) => {
    ...
    const apiKeyUsage = extractApiKeyUsageConfig(account);
    ...
    setEditForm({
      ...
      proxyUrl,
      usageProbeEnabled: apiKeyUsage.enabled,
      usageProbePath: apiKeyUsage.path,
      usageMinRemaining: apiKeyUsage.minRemaining,
    });
  };
  ...
  const saveEditPanel = async () => {
    ...
    try {
      await api.updateAccount(editingAccount.id, {
        ...
        apiKeyUsage:
          resolveAccountCredentialMode(editingAccount) === "apikey"
            ? {
                enabled: editForm.usageProbeEnabled,
                path: editForm.usageProbePath.trim() || "/v1/usage",
                minRemaining: Math.max(
                  0,
                  Number(editForm.usageMinRemaining) || 0,
                ),
              }
            : undefined,
      });
            ...
            {activeSegment === "session" ? (
              ...
              >
                ...
                >
                  <input
                    type="checkbox"
                    checked={tokenForm.usageProbeEnabled}
                    onChange={(e) =>
                      setTokenForm((f) => ({
                        ...f,
                        usageProbeEnabled: e.target.checked,
                      }))
                    }
                    style={{ width: 14, height: 14 }}
                  />
                ...
                {tokenForm.usageProbeEnabled && (
                  <ResponsiveFormGrid>
                    <input
                      placeholder="余额接口路径"
                      value={tokenForm.usageProbePath}
                      onChange={(e) =>
                        setTokenForm((f) => ({
                          ...f,
                          usageProbePath: e.target.value,
                        }))
                      }
                    ...
                    <input
                      ...
                      step="0.01"
                      placeholder="安全余额阈值"
                      value={tokenForm.usageMinRemaining}
                      onChange={(e) =>
                        setTokenForm((f) => ({
                          ...f,
                          usageMinRemaining: e.target.value,
                        }))
                      }
                      style={inputStyle}
                    />
                ...
                {resolveAccountCredentialMode(editingAccount) === "apikey" && (
                  <>
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={editForm.usageProbeEnabled}
                        onChange={(e) =>
                          setEditForm((prev) => ({
                            ...prev,
                            usageProbeEnabled: e.target.checked,
                    ...
                    {editForm.usageProbeEnabled && (
                      <>
                        <input
                          placeholder="余额接口路径"
                          value={editForm.usageProbePath}
                          onChange={(e) =>
                            setEditForm((prev) => ({
                              ...prev,
                              usageProbePath: e.target.value,
                        ...
                        <input
                          ...
                          step="0.01"
                          placeholder="安全余额阈值"
                          value={editForm.usageMinRemaining}
                          onChange={(e) =>
                            setEditForm((prev) => ({
                              ...prev,
                              usageMinRemaining: e.target.value,
