  ...
  const sessionCapable =
    credentialMode === "session"
      ...
      : credentialMode === "apikey"
        ? false
        : hasSessionToken;
  const apiKeyUsageCapable = credentialMode === "apikey"
    && typeof oauthIdentity === "object"
    && oauthIdentity !== null
    && getApiKeyUsageConfig(oauthIdentity.extraConfig)?.enabled === true;
  return {
    canCheckin: sessionCapable,
    canRefreshBalance: sessionCapable || apiKeyUsageCapable,
      ...
      if (parsedBody.data.apiKeyUsage !== undefined) {
        const baseExtraConfig =
          typeof updates.extraConfig === "string"
            ? updates.extraConfig
            : account.extraConfig;
        updates.extraConfig = mergeApiKeyUsageConfig(
          baseExtraConfig,
          parsedBody.data.apiKeyUsage,
        );
      }
