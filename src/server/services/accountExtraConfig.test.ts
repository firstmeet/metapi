...
import {
  ...
  getApiKeyUsageRoutingBlockReason,
...
describe('accountExtraConfig', () => {
  ...
  it('normalizes API Key usage probe configuration without executing scripts', () => {
    const extraConfig = mergeApiKeyUsageConfig(null, {
      enabled: true,
      path: '/v1/usage',
      minRemaining: 1.5,
      remaining: 8.25,
    ...
    expect(getApiKeyUsageConfig(extraConfig)).toEqual({
      enabled: true,
      path: '/v1/usage',
      minRemaining: 1.5,
      remaining: 8.25,
    ...
    expect(normalizeApiKeyUsagePath('https://evil.example/usage')).toBeUndefined();
    expect(normalizeApiKeyUsagePath('//evil.example/usage')).toBeUndefined();
  });

  it('blocks routing only after a successful probe confirms an invalid or low-balance package', () => {
    expect(getApiKeyUsageRoutingBlockReason(JSON.stringify({
