...
async function initializeAccountInBackground({
  ...
  const convergence = await convergeAccountMutation({
    ...
    refreshBalance: tokenType === 'session' || refreshApiKeyUsage === true,
...
function buildQueuedAccountInitializationMessage(
  ...
  if (skipModelFetch === true) {
    return refreshApiKeyUsage
      ? '已添加为 API Key 账号，后台正在同步套餐余额。'
...
export async function createManualAccount({
  ...
  const refreshApiKeyUsage = tokenType === 'apikey' && body.apiKeyUsage?.enabled === true;
  ...
  const shouldQueueInitialization = tokenType === 'session' || body.skipModelFetch !== true || refreshApiKeyUsage;
  ...
  if (shouldQueueInitialization) {
    ...
    queuedMessage = buildQueuedAccountInitializationMessage(tokenType, body.skipModelFetch, refreshApiKeyUsage);
  }
