...
async function refreshApiKeyUsageBalance(
  account: typeof schema.accounts.$inferSelect,
  site: typeof schema.sites.$inferSelect,
) {
  const usageConfig = getApiKeyUsageConfig(account.extraConfig);
  ...
  const apiKey = (account.apiToken || '').trim();
  if (!apiKey) {
    throw new Error('API Key usage probe requires an API Token');
  }

  const checkedAt = new Date().toISOString();
  const requestUrl = buildApiKeyUsageUrl(site.url, usageConfig.path);
  const accountProxyUrl = resolveProxyUrlFromExtraConfig(account.extraConfig);

  try {
    const snapshot = await probeApiKeyUsage({
      url: requestUrl,
      apiKey,
      requestInit: withSiteRecordProxyRequestInit(site, undefined, accountProxyUrl),
    });
    const used = account.balanceUsed ?? 0;
    const nextExtraConfig = mergeApiKeyUsageConfig(account.extraConfig, {
    ...
    await db.update(schema.accounts)
      .set({
        ...
        balanceUsed: used,
        quota: snapshot.remaining + used,
        extraConfig: nextExtraConfig,
        lastBalanceRefresh: checkedAt,
        updatedAt: checkedAt,
      })
      .where(eq(schema.accounts.id, account.id))
      .run();
    ...
    await setAccountRuntimeHealth(account.id, {
      ...
      reason: !snapshot.isValid
        ? '套餐已失效'
        : belowThreshold
          ? `套餐余额 ${snapshot.remaining} ${snapshot.unit} 低于安全阈值 ${usageConfig.minRemaining} ${snapshot.unit}`
          : 'API Key 套餐余额刷新成功',
      source: 'balance',
    });
    ...
    const isAuthFailure = error instanceof ApiKeyUsageProbeError
      && (error.status === 401 || error.status === 403);
    const message = error?.message || 'API Key usage probe failed';
    const nextExtraConfig = mergeApiKeyUsageConfig(account.extraConfig, {
      ...(isAuthFailure ? { isValid: false, checkedAt } : {}),
      lastError: message,
...
export async function refreshBalance(accountId: number) {
  ...
  if (isApiKeyConnection(account)) {
    return refreshApiKeyUsageBalance(account, site);
  }
