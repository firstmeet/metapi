...
describe('balanceService auto relogin', () => {
  ...
  it('refreshes package balance for API Key accounts with a usage probe', async () => {
    selectAllMock.mockReturnValue([
      {
        accounts: {
          id: 11,
          username: 'package-key',
          accessToken: '',
          apiToken: 'sk-package-token',
          balance: 5,
          balanceUsed: 1,
          quota: 6,
          status: 'active',
    ...
    expect(undiciFetchMock.mock.calls[0]?.[0]).toBe('https://relay.example.com/v1/usage');
    const usageRequest = undiciFetchMock.mock.calls[0]?.[1] as RequestInit;
    expect(usageRequest.method).toBe('GET');
    expect(new Headers(usageRequest.headers).get('authorization')).toBe('Bearer sk-package-token');
    expect(result).toEqual({
      balance: 0.75,
      used: 1,
      quota: 1.75,
    ...
    expect(updateSetMock).toHaveBeenCalledWith(expect.objectContaining({
      ...
      balanceUsed: 1,
      quota: 1.75,
      lastBalanceRefresh: expect.any(String),
      extraConfig: expect.stringContaining('"remaining":0.75'),
    }));
    expect(setAccountRuntimeHealthMock).toHaveBeenCalledWith(11, expect.objectContaining({
      state: 'degraded',
      source: 'balance',
    }));
