...
export type ApiKeyUsageConfigPatch = Omit<Partial<ApiKeyUsageConfig>, 'lastError'> & {
...
type AccountExtraConfig = {
  ...
  apiKeyUsage?: ApiKeyUsageConfigInput;
...
export function normalizeApiKeyUsagePath(raw: unknown): string | undefined {
  const path = normalizeNonEmptyString(raw);
...
export function getApiKeyUsageConfig(extraConfig?: ExtraConfigInput): ApiKeyUsageConfig | null {
  const parsed = parseExtraConfig(extraConfig);
  const raw = parsed.apiKeyUsage;
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;

  const config: ApiKeyUsageConfig = {
    enabled: raw.enabled === true,
    path: normalizeApiKeyUsagePath(raw.path) ?? DEFAULT_API_KEY_USAGE_PATH,
    minRemaining: normalizeNonNegativeNumber(raw.minRemaining) ?? 0,
  };
  const remaining = normalizeNonNegativeNumber(raw.remaining);
  if (remaining !== undefined) config.remaining = remaining;
  const unit = normalizeNonEmptyString(raw.unit);
  if (unit) config.unit = unit;
  if (typeof raw.isValid === 'boolean') config.isValid = raw.isValid;
  const checkedAt = normalizeIsoDateTime(raw.checkedAt);
  if (checkedAt) config.checkedAt = checkedAt;
  const lastError = normalizeNonEmptyString(raw.lastError);
  if (lastError) config.lastError = lastError;
  const lastErrorAt = normalizeIsoDateTime(raw.lastErrorAt);
  if (lastErrorAt) config.lastErrorAt = lastErrorAt;
  return config;
}

export function mergeApiKeyUsageConfig(
  extraConfig: ExtraConfigInput,
  patch: ApiKeyUsageConfigPatch,
): string {
  const parsed = parseExtraConfig(extraConfig);
  const existing = isRecord(parsed.apiKeyUsage) ? parsed.apiKeyUsage : {};
...
export function getApiKeyUsageRoutingBlockReason(extraConfig?: ExtraConfigInput): string | null {
  const usage = getApiKeyUsageConfig(extraConfig);
  if (!usage?.enabled || !usage.checkedAt) return null;
  if (usage.isValid === false) return '套餐已失效';
  if (usage.remaining !== undefined && usage.remaining <= usage.minRemaining) {
