...
describe('TokenRouter selection scoring', () => {
  ...
  it('excludes API Key connections whose probed balance reached the safety threshold', async () => {
    const route = await createRoute('gpt-5.4');
    const lowSite = await createSite('low-balance');
    const healthySite = await createSite('healthy-balance');
    const lowAccount = await createAccount(lowSite.id, 'low', {
    ...
    const healthyAccount = await createAccount(healthySite.id, 'healthy', {
      extraConfig: JSON.stringify({
        credentialMode: 'apikey',
        apiKeyUsage: {
          enabled: true,
    ...
    const selected = await new TokenRouter().selectChannel('gpt-5.4');
